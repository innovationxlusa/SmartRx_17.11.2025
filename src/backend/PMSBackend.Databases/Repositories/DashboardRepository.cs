    ...
    public class DashboardRepository : IDashboardRepository
    {
        ...
        public async Task<DashboardSummaryContract> GetDashboardSummaryAsync(long userId, long? patientId, CancellationToken cancellationToken)
        {
            ...
            var pendingQuery = _dbContext.Prescription_UploadedPrescription
                .AsNoTracking()
                .Where(p => p.UserId == userId && p.IsSmartRxRequested == true && (p.IsCompleted == null || p.IsCompleted == false));
            
            // Apply patientId filter if provided
            if (patientId.HasValue)
            {
                pendingQuery = pendingQuery.Where(p => p.PatientId == patientId.Value);
            }
            ...
            var rxFileOnlyQuery = _dbContext.Prescription_UploadedPrescription
              .AsNoTracking()
              .Where(p => p.UserId == userId                 
                  && !(p.IsSmartRxRequested !=null && p.IsSmartRxRequested == true && (p.IsCompleted == null || p.IsCompleted == false)));

            // Apply patientId filter if provided
            if (patientId.HasValue)
            {
                rxFileOnlyQuery = rxFileOnlyQuery.Where(p => p.PatientId == patientId.Value);
            }
            ...
            var totalMedicinesCost = await medicineQuery.SumAsync(
                m =>
                    ((m.Medicine != null ? (m.Medicine.UnitPrice ?? 0m) : 0m)) *
                    (
                        m.Dose1InADay +
                        m.Dose2InADay +
                        m.Dose3InADay +
