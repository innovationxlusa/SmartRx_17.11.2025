...
namespace PMSBackend.Databases.Repositories
{
    public class RewardPointConversionsRepository : IRewardPointConversionsRepository
    {
        ...
        public async Task<SmartRx_RewardPointConversions> CreateAsync(SmartRx_RewardPointConversions entity)
        {
        ...
        public async Task<SmartRx_RewardPointConversions?> GetByIdAsync(long id)
        {
        ...
        public async Task<IEnumerable<SmartRx_RewardPointConversions>> GetAllAsync()
        {
            return await _context.SmartRx_RewardPointConversions
                .AsNoTracking()
        ...
        public async Task<IEnumerable<SmartRx_RewardPointConversions>> GetByUserIdAsync(long userId)
        {
            return await _context.SmartRx_RewardPointConversions
                .AsNoTracking()
        ...
        public async Task<SmartRx_RewardPointConversions> UpdateAsync(SmartRx_RewardPointConversions entity)
        {
        ...
        public async Task<RewardPointConversionsSummaryContract> GetConversionSummaryByUserIdAsync(long userId)
        {
            ...
            var totalConvertedToCashable = conversions
                .Where(x => x.ToType == (int)RewardConvertionType.Cashable)
                .Sum(x => x.Amount);
                
            var totalConvertedToNonCashable = conversions
                .Where(x => x.ToType == (int)RewardConvertionType.Noncashable)
                .Sum(x => x.Amount);
                
            var totalConvertedToMoney = conversions
                .Where(x => x.ToType == (int)RewardConvertionType.Money)
                .Sum(x => x.Amount);

            // Calculate deductions
            var totalDeductedFromCashable = conversions
                .Where(x => x.FromType == (int)RewardConvertionType.Cashable)
                .Sum(x => x.Amount);
                
            var totalDeductedFromNonCashable = conversions
                .Where(x => x.FromType == (int)RewardConvertionType.Noncashable)
                .Sum(x => x.Amount);
                
            var totalDeductedFromMoney = conversions
                .Where(x => x.FromType == (int)RewardConvertionType.Money)
                .Sum(x => x.Amount);
