...
const RewardPoint = () => {
    ...
    const isValidUserId = loginUserId && !isNaN(loginUserId);
    ...
    const rewardData = useMemo(() => {
        if (!rewardApiData) {
            return {
                badgeId: 0,
                badgeName: "Basic Plus",

                totalEarnedNonCashablePoints: 0,
                totalConsumedNonCashablePoints: 0,
                totalNonCashablePoints: 0,

                totalEarnedCashablePoints: 0,
                totalConsumedCashablePoints: 0,
                totalCashablePoints: 0,

                totalEarnedMoney: 0,
                totalConsumedMoney: 0,
                totalMoney: 0,
                totalEncashMoney: 0,
                ...
                tiers: (() => {
                    ...
                    const currentBadgeName = "Basic Plus"; // Default
        ...
        return {
            userId: rewardApiData?.data?.userId || loginUserId,
            badgeId: rewardApiData?.data?.badgeId || 0,
            badgeName: rewardApiData?.data?.badgeName || "Basic Plus",
            patientId: rewardApiData?.data?.patientId || null,

            totalNonCashablePoints:
                rewardApiData?.data?.totalNonCashablePoints || 0,

            totalCashablePoints: rewardApiData?.data?.totalCashablePoints || 0,

            totalMoney: rewardApiData?.data?.totalMoney || 0,

            progress:
                Math.min(
                    100,
                    (rewardApiData?.data?.totalNonCashablePoints / 10000) * 100,
                ) || 0,
            tiers: (() => {
                const tierNames = [
                    "Basic Plus",
                ...
                const currentBadgeName =
                    rewardApiData?.data?.badgeName || "Basic Plus";
                const currentBadgeIndex = tierNames.findIndex(
                    (name) => name === currentBadgeName,
                );
    ...
    }, [rewardApiData, loginUserId]);

    const handlePointsChange = useCallback(
        (value) => {
            const maxPoints = rewardData?.totalCashablePoints ?? 0;
        ...
        [rewardData?.totalCashablePoints, conversionRate],
    );
    ...
    useEffect(() => {
        ...
        rewardData?.totalCashablePoints,
        pointsInput,
        conversionRate,
        handlePointsChange,
    ]);
    ...
    const onChangeNcToCPoints = (value) => {
        ...
        const max = rewardData?.totalNonCashablePoints || 0;
    ...
    const onChangeCToNcPoints = (value) => {
        ...
        const max = rewardData?.totalCashablePoints || 0;
        setCToNcPoints(clamp(value, 0, max));
    };

    const convertNcToC = async () => {
        if (!ncToCPoints) return;
        if (ncToCPoints > (rewardData?.totalNonCashablePoints || 0)) return;
        try {
            const payload = {
                userId: loginUserId,
                fromType: 1,
                toType: 2,
                amount: Number(ncToCPoints),
            };
            ...
            console.error("ConvertNonCashableToCashable failed", e);
    ...
    const convertCToNc = async () => {
        if (!cToNcPoints) return;
        if (cToNcPoints > (rewardData?.totalCashablePoints || 0)) return;
        try {
            const payload = {
                userId: loginUserId,
                fromType: 2,
                toType: 1,
                amount: Number(cToNcPoints),
            };
            await api.convertRewardPoints(
                payload,
            ...
            console.error("ConvertCashableToNonCashable failed", e);
            // Still refetch to ensure UI is up to date
            await refetchRewards?.();
        }
    };
                            ...
                            <div className="tabbed-rewards">
                                <div className="reward-tabs">
                                    ...
                                    >
                                        Non Cashable Points
                                        <br />
                                        {rewardData.totalNonCashablePoints}{" "}
                                        Total
                                    </button>
                                    <button
                                    ...
                                    >
                                        Cashable Points
                                        <br />
                                        {rewardData.totalCashablePoints} Total
                                    </button>
                                </div>
                                    ...
                                    <div className="tab-content mt-3">
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    >
                                                        {
                                                            rewardApiData?.data
                                                                ?.totalConsumedNonCashablePoints
                                                        }
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            >
                                                ...
                                                >
                                                    {rewardData.totalNonCashablePoints.toLocaleString()}
                                                </div>
                                            </div>
                                                ...
                                                <CustomButton
                                                    ...
                                                    onClick={async () => {
                                                        try {
                                                            ...
                                                            await refetchRewards?.();
                                                            alert(
                                                                `Successfully converted ${pointsInput} points to ${convertedAmount} Taka!`,
                                                            );
                                                        } catch (e) {
                                                            console.error(
