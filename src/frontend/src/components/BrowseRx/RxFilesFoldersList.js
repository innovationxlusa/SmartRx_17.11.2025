...
const RxFilesFoldersList = () => {
    ...
    const resolvedUserId = useMemo(() => {
        const fromContext = user?.UserId ?? user?.userId;
        const fromLocation = location.state?.UserId ?? location.state?.userId;
        const derived = Number(fromContext ?? fromLocation ?? 0);
    ...
    const handleDataRefresh = useCallback((movedFileId, actionType, renamedValue) => {
        ...
        const trimmedRename =
            shouldApplyRename ? renamedValue.trim() : undefined;
        ...
        const applyRenameToItem = (item) => {
            ...
            return {
                ...item,
                folderOrFileName:
                    trimmedRename ?? item?.folderOrFileName ?? item?.folderName,
                folderName:
                    trimmedRename ?? item?.folderName ?? item?.folderOrFileName,
        ...
        if (shouldApplyRename) {
            ...
            setFolderAndFileData((prev) => {
                ...
                const nextChildrenData = applyRenameToList(prevChildrenData);
                const childrenChanged = nextChildrenData !== prevChildrenData;
        ...
        if (shouldPrune) {
            ...
            setFolderAndFileData((prev) => {
                ...
                const updatedPrescriptions = pruneList(currentPrescriptions);
                ...
                return {
                    ...
                    children: prev.children
                        ? {
                              ...
                              totalRecords:
                                  typeof prev.children.totalRecords === "number"
                                      ? Math.max(
                                            0,
    ...
    const getDisplayName = useCallback((name) => {
        if (!name) return name;
        return String(name).trim().toLowerCase() === 'primary' ? 'Rx Folder' : name;
    }, []);
    ...
    const findFolderNameInTree = useCallback((node, targetId) => {
        ...
        if (node.folderId === targetId) return node.folderOrFileName || node.folderName || null;
    ...
    const getFolderNameById = useCallback((id) => {
        ...
        if (stateParent && stateParent.folderId === id) {
            return stateParent.folderOrFileName || stateParent.folderName || null;
        }
    ...
    const getItemKey = useCallback((item) => {
        ...
        if (isFolder) {
            ...
            const name = item?.folderOrFileName || item?.folderName || item?.name;
            const parent = item?.parentFolderId ?? 'root';
            if (name) return `folder-name-${parent}-${String(name).toLowerCase()}`;
        }
        // Files
        const isFile = item?.isFolder === false || item?.type === 'file';
        if (isFile) {
            if (item?.fileId != null) return `file-${item.fileId}`;
            const fname = item?.fileName || item?.folderOrFileName || item?.name;
            const created = item?.createdDateStr || item?.createdDate || item?.uploadedAt || '';
            if (fname) return `file-name-${String(fname).toLowerCase()}-${String(created)}`;
        }
        // Prescriptions (Patient View)
        if (item?.type === 'prescription') {
            if (item?.prescriptionId != null) return `rx-${item.prescriptionId}`;
            ...
            if (pname || created) return `rx-name-${owner}-${String(pname).toLowerCase()}-${String(created)}`;
        }
        // Generic fallback
    ...
    }, [patientId, folderId, currentFolder, currentPath, debouncedSearch, sortBy, currentPage, itemsPerPage, refreshKey, resolvedUserId]);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (response?.message === "Successful") {
                ...
                if (resp && resp.parentFolderId == null && typeof resp.folderId === 'number') {
                    setRootFolderId(prev => (prev !== resp.folderId ? resp.folderId : prev));
                }
                if (!isPatientView && !childrenData) {
                    const children = Array.isArray(resp?.children?.data) ? resp.children.data : [];
                    const files = Array.isArray(resp?.prescriptionList) ? resp.prescriptionList : [];
                    ...
                    const fileMapped = files.map((file, fidx) => ({
                        ...
                        type: 'file',
                        isFolder: false,
                        folderOrFileName: file.folderOrFileName || file.fileName || file.name,
                    ...
                    setApiChildrenItems((prev) => {
                        if (currentPage === 1 || !Array.isArray(prev) || prev.length === 0) {
                            nextAggregatedItems = mapped;
                            return mapped;
                        }
                        const base = Array.isArray(prev) ? prev : [];
                        const seen = new Set(base.map(getItemKey).filter(Boolean));
                        ...
                        nextAggregatedItems = appended.length ? [...base, ...appended] : base;
                        return nextAggregatedItems;
                    });
                    setVisibleItemsState(Array.isArray(nextAggregatedItems) ? nextAggregatedItems : []);
                }
            } else {
                setFolderAndFileData(null);
                setCombinedItems([]);
    ...
    }, [browsePayload, itemsPerPage, currentPage, sortBy, sortDirection, debouncedSearch, childrenData, getItemKey]); // stable, memoized
    ...
    useEffect(() => {
        ...
        const folderInfo = {
            ...
            folderOrFileName: getDisplayName(folderAndFileData.folderOrFileName || folderAndFileData.folderName || `Folder ${folderId}`),
    ...
    }, [folderId, folderAndFileData?.folderOrFileName, folderAndFileData?.children?.totalRecords, childrenData]);

    useEffect(() => {
        if (location.state && folderId) {
            const { folderId: stateFolderId, folderName, childrenData: stateChildrenData, parentFolder, parentFolderId: stateParentFolderId, selectedChild } = location.state;
            ...
            if (stateChildrenData && Array.isArray(stateChildrenData)) {
                ...
                const folderInfo = selectedChild ? {
                    ...
                    folderOrFileName: getDisplayName(selectedChild.folderOrFileName || folderName),
                    ...
                    folderHeirarchy: selectedChild.folderHeirarchy ?? ((parentFolder?.folderHeirarchy || 0) + 1),
                    ...
                    parentFolderId: (parentFolder?.parentFolderId ? parentFolder.parentFolderId : null),
    ...
    const shouldCallPatientPrescriptions = isPatientView && Boolean(resolvedUserId);
    ...
    const totalAvailableCount = useMemo(() => {
        if (isPatientView) {
            ...
            if (childrenData && Array.isArray(childrenData)) {
                ...
                const childCount = Number(folderAndFileData?.children?.totalRecords ?? 0) || 0;
                const fileCount = Array.isArray(folderAndFileData?.prescriptionList) ? folderAndFileData.prescriptionList.length : 0;
    ...
    }, [isPatientView, childrenData, apiChildrenItems, patientPrescriptionsData?.data, sortBy, debouncedSearch]);

    useEffect(() => {
        if (isPatientView) {
            setVisibleItemsState(prev => (listsEqualByKey(prev, allCombinedItems) ? prev : allCombinedItems));
            return;
        }
