...
const Overview = ({ smartRxInsiderData, fileId, refetch }) => {
    ...
    const totalMedicinesPrescribed = useMemo(() => {
        const medicines = smartRxInsiderData?.prescriptions?.[0]?.medicines;

        if (Array.isArray(medicines)) {
            return medicines.length;
        }

        const parsed = Number(medicines);
        return Number.isFinite(parsed) && parsed >= 0 ? parsed : 0;
    }, [smartRxInsiderData?.prescriptions]);

    const totalInvestigationsOrdered = useMemo(() => {
        ...
        const parsed = Number(investigations);
        return Number.isFinite(parsed) && parsed >= 0 ? parsed : 0;
    }, [smartRxInsiderData?.prescriptions]);
    ...
    return (
        <>
                    ...
                    <div className="text-start">
                        ...
                        <div className="d-flex justify-content-between align-items-center px-3">
                            <div className="custom-card-title">Prescribed <br />Medication</div>
                                <div className="custom-card-title-2 text-center"><span>{totalMedicinesPrescribed}</span> <br /> <span>Medicine</span></div>
                ...
                <CustomCard>
                    <div className="text-start">
                        ...
                        <div className="d-flex justify-content-between align-items-center px-3">
                            ...
                            <div className="custom-card-title-2 text-center"><span>{totalInvestigationsOrdered}</span> <br /> <span>Tests</span></div>
                                ...
                                <div className="cost-time-row">
                                    ...
                                    <div className="cost-time-input-group">
                                        <input 
                                            ...
                                            className="cost-time-input"
                                            value={transportFee}
                                            onChange={(e) => setTransportFee(e.target.value === '' ? 0 : Number(e.target.value) || 0)}
                                        />
                                        <span className="cost-time-unit">
            ...
            <DoctorReviewModal
                ...
                refetch={refetch}
                smartRxOverview={smartRxInsiderData?.smartRxOverview}
                doctorRating={smartRxInsiderData?.prescriptions?.[0]?.doctor?.doctorRating}
                doctorComments={smartRxInsiderData?.prescriptions?.[0]?.doctor?.doctorComments || smartRxInsiderData?.prescriptions?.[0]?.doctor?.comments}
            />
        </>
    )
}
