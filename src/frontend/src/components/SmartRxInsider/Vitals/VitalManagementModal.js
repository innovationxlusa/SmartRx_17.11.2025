...
const VitalManagementModal = ({
    ...
    const userId = useMemo(() => {
        const contextUserId = user?.UserId ?? user?.userId;
        const navigationUserId = location.state?.UserId ?? location.state?.userId;
        const derivedId = Number(contextUserId ?? navigationUserId ?? 0);
        return Number.isFinite(derivedId) && derivedId > 0 ? derivedId : null;
    }, [user, location.state]);
    // Destructuring API service methods
    const { api } = useApiClients();
    console.log("Patient Id: ", patientId); 
    console.log("Prescription Id: ", prescriptionId);
    console.log("FolderId- Prescriptionid ", folderData);
    console.log("Smartrxid: ", smartRxMasterId);    
    // Destructuring API service methods
    ...
    useEffect(() => {
        const fetchVitalEntity = async () => {
            if (!formData.VitalName || typeof fetchSmartRxVitalData !== "function") return;
    ...
    const handleSubmit = async (e) => {
        ...
        if (modalType !== "delete") {
            const fieldsToValidate = {
                VitalName: validateField("VitalName", formData.VitalName, "Vital Name",),
                VitalValue: formData.VitalName !== "bp" ? validateField("VitalValue", formData.VitalValue, "Vital Reading",) : "",
                DiastolicValue: formData.VitalName === "bp" ? validateField("DiastolicValue", formData.DiastolicValue, "Diastolic",) : "",
                SystolicValue: formData.VitalName === "bp" ? validateField("SystolicValue", formData.SystolicValue, "Systolic",) : "",
                VitalUnit: validateField("VitalUnit", formData.VitalUnit, "Vital Measurement Unit", ),
            };
