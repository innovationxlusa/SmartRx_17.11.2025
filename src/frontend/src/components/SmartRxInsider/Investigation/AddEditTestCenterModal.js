...
const AddEditTestCenterModal = ({
    ...
    const getTestSelectionCount = useCallback(
        (testName) =>
            Object.keys(selectedTests).reduce(
                (count, key) =>
                    key.endsWith(`_${testName}`) && selectedTests[key]
                        ? count + 1
                        : count,
                0,
            ),
    ...
    const handleRemoveDoctorRecommendedCenter = useCallback(
        (centerIdToRemove) => {
    ...
    const handleCenterBulkSelection = useCallback(
        (testKeys, allSelected) => {
    ...
    // Doctor recommended centers data
    const doctorRecommendedCentersData = useMemo(() => {
        ...
        uniqueTestCenters.forEach((center) => {
            centerIdToName[center.testCenterId?.toString()] =
                center.testCenterName;
        });

        const centerIdMap = new Map();
        for (const test of allTests) {
            const idsArr = (test.testCenterIds || "")
                .split(",")
                .map((id) => id.trim());
            idsArr.forEach((id) => {
                if (!id) return;
                ...
                if (
                    !centerIdMap
                        .get(id)
                        .some((t) => t.testName === test.testName)
                ) {
        ...
        return Array.from(centerIdMap.entries())
            .map(([centerId, filteredTests]) => {
                ...
                const allSelected =
                    testKeys.length > 0 &&
                    testKeys.every((key) => selectedTests[key]);
    ...
    const totalCentersCount =
        doctorRecommendedCentersData.length + addedCenters.length;
    ...
    const handleAddCenter = useCallback(() => {
        ...
        setAddedCenters((prev) => [
            ...prev,
            { center: selectedCenter, tests: relatedTests },
        ]);
        resetForm(
            { DiagnosticCenterName: "", Branch: "", InvestigationId: "" },
            setFormData,
            setFieldErrors,
        );
    }, [
            ...
            (doctorRecommendedTestCenterAndTestListData || []).forEach(
                ({ tests }) => {
                    (tests || []).forEach((test) => {
                        ...
                        ids.forEach((id) => {
                            initialSelections[`${id}_${test.testName}`] = true;
                        });
                    });
                },
            );
        ...
        try {
            ...
            doctorRecommendedCenters.forEach(({ tests }) => {
                tests?.forEach((test) => {
                    test.testCenterIds
                        ?.split(",")
                        .forEach((cid) => processTest(cid.trim(), test));
                });
            });
            ...
            await api.investigationCenterListUpdate(
                {
                    SmartRxMasterId,
                    PrescriptionId,
            ...
            return (
                ...
                >
                    <input
                        ...
                        checked={isChecked}
                        onChange={() =>
                            toggleTestSelection(centerId, test.testName)
                        }
        ...
        ({
            ...
            >
                <input
                    ...
                    checked={anySelected}
                    onChange={() =>
                        handleCenterBulkSelection(testKeys, allSelected)
                    }
                    style={{
                        accentColor: "#4b3b8b",
                        width: "clamp(5%, 30vw, 5%)",
                ...
                />
                <strong style={{ color: "#65636e", fontSize: "16px" }}>
                    {centerName}
                </strong>
        ...
        >
            <div className="fade-in">
                ...
                <div style={{ marginTop: "2px", textAlign: "left" }}>
                    ...
                    {doctorRecommendedCentersData.map(
                        ({
                            centerId,
                            centerName,
                            filteredTests,
                            ...
                            >
                                <CenterHeader
                                    ...
                                    testKeys={testKeys}
                                    onRemove={() =>
                                        handleRemoveDoctorRecommendedCenter(
                                            centerId,
                                        )
                                    }
                                />
                    ...
                    {addedCenters.map(({ center, tests }) => {
                        ...
                        return (
                            ...
                            >
                                <CenterHeader
                                    centerId={centerId}
                                    centerName={center.testCenterName}
                                    anySelected={anySelected}
                                    allSelected={allSelected}
                                    testKeys={testKeys}
                                    onRemove={() =>
