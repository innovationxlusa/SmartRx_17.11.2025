...
const Dashboard = () => {
    ...
    const hasFilesFromLogin = Boolean(location.state?.hasFiles);
    ...
    useEffect(() => {
        ...
        if (hasFilesFromLogin) {
            smartNavigate("/all-patient", { replace: true });
        }
    }, [accessToken, hasFilesFromLogin, smartNavigate]);
